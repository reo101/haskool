...
lexer input
  ...
  | isDigit (T.head input) = TInteger numText : lexer (T.drop (T.length numText) input)
  -- | isDigit (T.head input) = case maybeRead (T.unpack numText) of
  --     Just num -> TInteger num : lexer (T.drop (T.length numText) input)
  --     Nothing -> TError "Invalid number" : lexer (T.tail input)
  | otherwise = TError ("Invalid character: " <> T.pack [T.head input]) : lexer (T.tail input)
 where
  symbolToken = case T.head input of
    ...
    ')' -> TCloseParen
    '=' ->
      if T.head (T.tail input) == '='
        then TEqual
        else TError "Invalid token: ="
    _ -> TError $ "Invalid character: " <> T.pack [T.head input]
